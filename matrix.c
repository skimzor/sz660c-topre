...
/*
 * Pin configuration for ATMega32U4 SZ660C
 *
 * Row:     PF4-6, 7(~EN)
 ...
 * Hys:     PC7
 */
static inline void KEY_ENABLE(void) { (PORTF &= ~(1<<7)); }
static inline void KEY_UNABLE(void) { (PORTF |=  (1<<7)); }
...
static inline void SET_ROW(uint8_t ROW)
{
    ...
    PORTF = (PORTF & 0x0F) | (1<<7) | ((ROW & 0x07) << 4);
}
